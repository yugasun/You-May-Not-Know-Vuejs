...
当我们点击按钮时，将 `show` 置为 `true`，此时 `v-if` 指令就判断显示使用 `v-count-down` 指令的元素，然后执行 `bind` 初始化函数，将元素内容修改为 `60`。这里很好理解。但是我们要实现倒计时，就需要创建一个定时器，并需要维护一个数实现递减，这里通过动态修改 `data-count` 的值来实现。
